var sketchWithParams = function(date, message, n_pixels, font_path, out_path) {
  ...
  let sketch = function(p, preloaded) {
    ...
    p.preload = function() {
      let box1 = p.loadImage('flovatar/box1.png')
      let box2 = p.loadImage('flovatar/box2.png')
      let box3 = p.loadImage('flovatar/box3.png')
      let box4 = p.loadImage('flovatar/box4.png')
      let box5 = p.loadImage('flovatar/box5.png')
      stick_imgs = [box1, box2, box3, box4, box5]

      let ibox1 = p.loadImage('flovatar/ibox1.png')
      let ibox2 = p.loadImage('flovatar/ibox2.png')
      let ibox3 = p.loadImage('flovatar/ibox3.png')
      let ibox4 = p.loadImage('flovatar/ibox4.png')
      let ibox5 = p.loadImage('flovatar/ibox5.png')
      let globe = p.loadImage('flovatar/globe.png')
      let flova1 = p.loadImage('flovatar/flova1.png')
      let flova2 = p.loadImage('flovatar/flova2.png')
      let flova3 = p.loadImage('flovatar/flova3.png')
      bottom_imgs = [ibox1, ibox2, ibox3, ibox4, ibox5, globe, flova1, flova2, flova3]
    
      let weapon1 = p.loadImage('flovatar/weapon1.png')
      let weapon2 = p.loadImage('flovatar/weapon2.png')
      weapon_imgs = [weapon1, weapon2]
    ...
    p.setup = function() {
      ...
      if (font_out != null) {
        stick_imgs = [preloaded.box1, preloaded.box2, preloaded.box3, preloaded.box4, preloaded.box5]  

        bottom_imgs = [preloaded.ibox1, preloaded.ibox2, preloaded.ibox3, preloaded.ibox4, preloaded.ibox5, preloaded.globe, preloaded.flova1, preloaded.flova2, preloaded.flova3]    

        weapon_imgs = [preloaded.weapon1, preloaded.weapon2] 
      }
    ...
    function drawStick(attach_point, level, intersection, len, xs, ys, vs) {
      var stick_top = null
      if(attach_point + level > 20 && rand() > 0.2) {    
    ...
    function renderBoard(){
      ...
      let pos = parseInt(rand() * bottom_imgs.length, 10)
      let img = bottom_imgs[pos]
      let scaling = 5
      if(max_fill_y < p.height * 0.9 && max_fill_y - min_fill_y > img.height / scaling * res_ratio && max_cond_fill_x >= 12 && rand() > 0.2){        
        p.image(img, p.width/2 - img.width / (scaling * 2.2) * res_ratio, max_fill_y - img.height / (scaling * 0.9) * res_ratio, img.width / scaling * res_ratio, img.height / scaling * res_ratio);
      }
      // draw stick top
      if (stick_top != null) {      
        ...
        let pos = parseInt(rand() * stick_imgs.length, 10)
        let img = stick_imgs[pos]
        p.image(img, 0, 0, img.width / 8 * res_ratio, img.height / 8 * res_ratio);
        p.pop()
        
      }
      if(isCurved && rand() > 0.5) {
        let pos = parseInt(rand() * weapon_imgs.length, 10)
        let weapon = weapon_imgs[pos]
        scaling = 2
        var shift = pos == 1? 0.6 : 0
        p.image(weapon, p.width/2 - weapon.width / (scaling * (3.5 + shift)) * res_ratio, p.height/4, weapon.width / scaling * res_ratio, weapon.height / scaling * res_ratio);
      }
